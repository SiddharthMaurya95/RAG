...
def execute_sql(conn, query):
    """Legacy direct execution wrapper if needed elsewhere."""
    if conn is None:
...
- Map misspelled/short columns correctly:
  ...
  department/dept/dept. -> department_of_action_judgement, days -> days_used,
  person/investigator -> person_of_action_judgement, judgement date -> judgement_date,
  approval date -> approval_judgement_date.
- Whenever "department" is concerned or mentioned, ALWAYS use the 'department_of_action_judgement' column.
- 'MQ' or 'mq' represents a department and maps to the expanded prefix 'Quality Assurance Market Quality'. If the query mentions 'MQ [Number]' or 'mq [Number]' (e.g. 'MQ 7', 'MQ 6'), interpret it as 'Quality Assurance Market Quality-[Number]' (with a hyphen) when filtering on the 'department_of_action_judgement' column (e.g. department_of_action_judgement = 'Quality Assurance Market Quality-7').
...
Output: SELECT * FROM records WHERE department_of_action_judgement = 'Quality Assurance Market Quality-7'
